...
OpenSimplexNoise::OpenSimplexNoise(short *perm)
{
	this->m_perm = perm;
	this->m_permGradIndex3D = new short[PERMUTATION_LENGTH];

	//Since 3D has 24 gradients, simple bitmask won't work, so precompute modulo array.
	for (int n = 0; n < PERMUTATION_LENGTH; n++)
		this->m_permGradIndex3D[n] = static_cast<short>((perm[n] % (gradients3DLength / 3)) * 3);
}

OpenSimplexNoise::OpenSimplexNoise(long seed)
{
	this->m_perm = new short[PERMUTATION_LENGTH];
	this->m_permGradIndex3D = new short[PERMUTATION_LENGTH];
	short source[PERMUTATION_LENGTH];
	for (short n = 0; n < PERMUTATION_LENGTH; n++)
		source[n] = n;
	...
	for (int n = PERMUTATION_LENGTH - 1; n >= 0; n--)
	{
		seed = seed * 6364136223846793005l + 1442695040888963407l;
		int r = static_cast<int>((seed + 31) % (n + 1));
		if (r < 0)
			r += (n + 1);

		this->m_perm[n] = source[r];
		this->m_permGradIndex3D[n] = static_cast<short>((this->m_perm[n] % (gradients3DLength / 3)) * 3);
		source[r] = source[n];
	}
}
...
{
	delete[] this->m_perm;
	delete[] this->m_permGradIndex3D;
}

double OpenSimplexNoise::extrapolate(const int xsb, const int ysb, const double dx, const double dy) const
{
	short index = this->m_perm[(this->m_perm[xsb & 0xFF] + ysb) & 0xFF] & 0x0E;
	return std::fma(gradients2D[index], dx, gradients2D[index + 1] * dy);
}

double OpenSimplexNoise::extrapolate(const int xsb, const int ysb, const int zsb, const double dx, const double dy, const double dz) const
{
	short index = this->m_permGradIndex3D[(this->m_perm[(this->m_perm[xsb & 0xFF] + ysb) & 0xFF] + zsb) & 0xFF];
	return std::fma(gradients3D[index], dx, std::fma(gradients3D[index + 1], dy, gradients3D[index + 2] * dz));
}

double OpenSimplexNoise::extrapolate(const int xsb, const int ysb, const int zsb, const int wsb, const double dx, const double dy, const double dz,
									 const double dw) const
{
	short index = this->m_perm[(this->m_perm[(this->m_perm[(this->m_perm[xsb & 0xFF] + ysb) & 0xFF] + zsb) & 0xFF] + wsb) & 0xFF] & 0xFC;
	return std::fma(gradients4D[index], dx, std::fma(gradients4D[index + 1], dy, std::fma(gradients4D[index + 2], dz, gradients4D[index + 3] * dw)));
}

double OpenSimplexNoise::eval(const double x, const double y) const
{
...
double OpenSimplexNoise::eval(const double x, const double y, const double z) const
{
	...
	if (inSum <= 1) { //We're inside the tetrahedron (3-Simplex) at (0,0,0)
		...
		unsigned int aPoint = 0x01U;
		double aScore = xins;
		unsigned int bPoint = 0x02U;
		double bScore = yins;
		...
		if (wins > aScore || wins > bScore) { //(0,0,0) is one of the closest two tetrahedral vertices.
			unsigned int c = (bScore > aScore ? bPoint : aPoint); //Our other closest vertex is the closest out of a and b.
			...
			unsigned int c = (aPoint | bPoint); //Our two extra vertices are determined by the closest two.
		...
		unsigned int aPoint = 0x06U;
		double aScore = xins;
		unsigned int bPoint = 0x05U;
		double bScore = yins;
		if (aScore <= bScore && zins < bScore) {
			bScore = zins;
			bPoint = 0x03U;
...
double OpenSimplexNoise::eval(const double x, const double y, const double z, const double w) const
{
	//Place input coordinates on simplectic honeycomb.
	...
	if (inSum <= 1) { //We're inside the pentachoron (4-Simplex) at (0,0,0,0)
		...
		if (uins > aScore || uins > bScore) { //(0,0,0,0) is one of the closest two pentachoron vertices.
			...
			unsigned int c = aPoint | bPoint; //Our three extra vertices are determined by the closest two.
		...
		unsigned int aPoint = 0x0EU;
		double aScore = xins;
		unsigned int bPoint = 0x0DU;
		double bScore = yins;
		if (aScore <= bScore && zins < bScore) {
			bScore = zins;
			bPoint = 0x0BU;
		...
		if (aIsBiggerSide == bIsBiggerSide) {
			if (aIsBiggerSide) { //Both closest points on the bigger side
				unsigned int c1 = aPoint | bPoint;
				unsigned int c2 = aPoint & bPoint;
				if ((c1 & 0x01U) == 0x0U) {
					xsv_ext0 = xsb;
					xsv_ext1 = xsb - 1;
